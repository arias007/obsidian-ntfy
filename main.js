...
var NtfyLanSyncRuntime = (() => {
  ...
  var ACTIVE_EDIT_SYNC_DELAY_MS = 50;
  var ACTIVE_EDIT_MIN_INTERVAL_MS = 100;
  ...
  var TRANSFER_ABORT_FAILURE_STREAK = 6;
  ...
  var INCREMENTAL_PATH_BATCH_SIZE = 1e5;
...
${bodyHash}`;
  ...
  function prioritizeLanSyncActions(actions, context = {}) {
    ...
    const localDirty = context.localDirty ?? /* @__PURE__ */ new Map();
    const remoteDirty = context.remoteDirty ?? /* @__PURE__ */ new Map();
    const rank = (action) => {
  ...
  var NtfyLanSync = class {
    ...
    progressUpdatedAt = 0;
    urgentDirtyPaths = /* @__PURE__ */ new Set();
    ...
    inboundSession = null;
    appliedMutationEvents = /* @__PURE__ */ new Map();
    servedFilesystemScanRequests = /* @__PURE__ */ new Map();
    ...
    activityFiles = [];
    visibleCandidatePaths = /* @__PURE__ */ new Set();
    ...
    async sendDeviceFile(deviceId, input) {
      ...
      if (!peer.canHost) {
        ...
        return {
          ...
          hash,
          temporary: true,
          expiresAt: new Date(this.now() + this.settings().inboxRetentionHours * 60 * 6e4).toISOString()
        };
      }
    ...
    async stop() {
      ...
      if (this.backgroundReconciliation) {
        await this.backgroundReconciliation.catch(() => void 0);
    ...
    savePendingMessages() {
      try {
        const peers = Object.fromEntries(
          [...this.pendingMessages.entries()].filter(([, messages]) => messages.length).map(([deviceId, messages]) => [deviceId, messages.slice(-MAX_QUEUED_MESSAGES_PER_PEER)])
        );
        this.localStore()?.setItem(this.outboundMessageStorageKey(), JSON.stringify({ schemaVersion: 1, peers }));
      } catch {
      }
    }
    outboundAttachmentPath(deviceId, attachmentPath) {
...
`);
    ...
    finishInboundFileActivity(deviceId, index, success, path = "", payload = {}) {
      const expectedPath = path.endsWith("/attachment/write") ? normalizeLanInboxAttachmentPath(`${LAN_INBOX_ROOT}/${String(payload.attachmentId || "")}/${safeLanInboxName(payload.name)}`) : this.normalizePath(payload.path);
    ...
    async receiveQueuedMessages(peer, value) {
      ...
      for (const item of value) {
        try {
          ...
          if (!this.receivedMessageIds.has(message.id)) {
            ...
            if (this.receivedMessageIds.size > 5e3) this.receivedMessageIds.delete(this.receivedMessageIds.values().next().value ?? "");
          }
    ...
    async handleMessageAcknowledgement(deviceId, payload) {
      ...
      for (const message of removed) {
        for (const attachment of message.attachments) {
          const sourcePath = this.outboundAttachmentPath(deviceId, attachment.path);
          if (sourcePath && await this.options.storage.exists(sourcePath)) await this.options.storage.deleteFile(sourcePath).catch(() => void 0);
        }
      }
    ...
    parseScanRequestIds(value) {
      if (!Array.isArray(value)) return [];
      return [...new Set(value.filter((item) => typeof item === "string" && /^[A-Za-z0-9_-]{12,96}$/.test(item)).slice(0, 4))];
    }
    filesystemScanAlreadyServed(deviceId, requestIds) {
    ...
    recordServedFilesystemScan(deviceId, requestIds) {
      if (!requestIds.length) return;
      const previous = this.servedFilesystemScanRequests.get(deviceId) ?? [];
      this.servedFilesystemScanRequests.set(deviceId, [.../* @__PURE__ */ new Set([...previous, ...requestIds])].slice(-16));
    }
    metadataProtocol(peer) {
      return METADATA_PROTOCOLS.find((protocol) => peer.capabilities.has(protocol.capability)) ?? null;
    }
    metadataRoute(peer, suffix) {
    ...
    parseRemoteProgress(value) {
      if (!isRecord(value)) return null;
      const phases = ["stopped", "discovering", "connected", "scanning", "syncing", "complete", "error"];
    ...
    mirrorRemoteProgress(peer) {
      ...
      if (remote.phase !== "syncing") {
        this.emit({
          ...defaultProgress("connected"),
          stage: remote.stage === "fingerprinting" ? "fingerprinting" : remote.stage === "packaging-manifest" ? "packaging-manifest" : remote.stage === "planning" ? "planning" : remote.stage === "waiting-plan" ? "waiting-plan" : "enumerating",
          active: true,
          peerId: peer.deviceId
        });
        return;
    ...
    async verifyPeer(peer, force = false) {
      ...
      try {
        ...
        peer.lastFailureAt = 0;
        this.lastErrorValue = "";
        if (this.progressValue.phase !== "scanning" && this.progressValue.phase !== "syncing" && this.progressValue.phase !== "complete" && !["enumerating", "fingerprinting", "packaging-manifest", "planning", "waiting-plan"].includes(this.progressValue.stage)) {
    ...
    sweepPeers() {
      ...
      if (this.progressValue.phase === "syncing" && !this.syncRunning && !this.inboundSession && !hasActiveTransfer && this.now() - this.lastTransferAt > TRANSFER_IDLE_RESET_MS) {
    ...
    async syncPeerMetadata(peer, request) {
      ...
      let failure = null;
      let failureStreak = 0;
      const retryPaths = /* @__PURE__ */ new Set();
    ...
    async buildMetadataHashManifest(expectedEntries, includeConfigFolder, onProgress) {
    ...
    canExposeScanProgress() {
      if (this.backgroundReconciliation) return false;
      if (this.inboundManifestDepth === 0) return true;
      return !this.syncRunning && this.scanValue.phase !== "scanning" && this.progressValue.phase !== "syncing";
    }
    ...
    async buildMetadataManifest(includeConfigFolder = this.settings().syncConfigFolder, onProgress, forceFilesystemScan = false) {
      if (this.metadataManifestBuild) {
        ...
        if (coversConfigFolder && coversFilesystemScan) {
          ...
          return activeBuild.includeConfigFolder === includeConfigFolder ? reused : reused.filter((entry) => this.normalizePath(entry.path, includeConfigFolder) !== null);
        }
    ...
    async readRemoteMetadata(peer, entry, sessionId = "") {
    ...
    async callPeer(peer, route, payload, timeoutMs = 8e3) {
      ...
      if (route === "/ping" && addresses.length > 1) {
        ...
        try {
          ...
          const protocolError = reasons.find((reason) => reason instanceof LanSyncProtocolError);
...
class NtfyLanSyncDetailsModal extends Modal {
  constructor(app, getSnapshot, isChinese, requestSync, onClosed) {
    ...
    this.closed = true;
    this.sectionState = { history: false, scan: false, transfer: false };
  ...
  render() {
    ...
    const stageLabels = chinese
      ...
      : {
          ...
          "waiting-plan": "Manifest sent; waiting for transfer plan",
    ...
    const currentStatus = body.createDiv({ cls: "obsidian-ntfy-lan-current-status" });
    const summary = currentStatus.createDiv({ cls: "obsidian-ntfy-lan-details-summary" });
    const summaryIcon = summary.createSpan({ cls: "obsidian-ntfy-lan-details-summary-icon" });
    // Keep the LAN panel calm: the Wi-Fi mark communicates the connection;
    // numeric progress below communicates work. Avoid a constantly restarting
    // green spinner and red error decoration while the panel refreshes.
    setIcon(summaryIcon, "wifi");
    // Kept as a compatibility no-op for older embedded UI integrations; the
    // summary now uses a static Wi-Fi icon, so this branch never runs.
    ...
    if (scan.total > 0) progressParts.push(
      `${chinese ? "本机已扫描" : "Local scanned"} ${scan.completed || 0} / ${chinese ? "本轮总扫描" : "round total"} ${scan.total || 0}`
    );
    if (remote && (remote.scanPhase === "scanning" || remote.scanTotal > 0)) {
      progressParts.push(`${chinese ? "对端已扫描" : "Peer scanned"} ${remote.scanCompleted || 0} / ${chinese ? "本轮总扫描" : "round total"} ${remote.scanTotal || 0}`);
  ...
  renderRoundHistory(body, history, chinese) {
    ...
    details.open = this.sectionState.history;
    details.addEventListener("toggle", () => {
      if (this.sectionState.history === details.open) return;
      this.sectionState.history = details.open;
      this.refresh();
    });
    ...
    for (const round of [...safeHistory].reverse()) {
      ...
      const heading = row.createDiv({ cls: "obsidian-ntfy-lan-round-history-heading" });
      heading.createEl("strong", { text: `${started} → ${finished}` });
      const statusLabel = status === "error"
        ? (chinese ? "错误" : "Error")
        : status === "partial"
          ? (chinese ? "部分完成" : "Partial")
          : (chinese ? "已完成" : "Complete");
      heading.createSpan({ cls: "obsidian-ntfy-lan-details-section-meta", text: statusLabel });

      const content = row.createDiv({ cls: "obsidian-ntfy-lan-round-history-content" });
      const localScanCompleted = safeNumber(round.localScanCompleted);
      const localScanTotal = safeNumber(round.localScanTotal);
      const remoteScanCompleted = safeNumber(round.remoteScanCompleted);
      const remoteScanTotal = safeNumber(round.remoteScanTotal);
      const syncCompleted = safeNumber(round.syncCompleted);
      const syncTotal = safeNumber(round.syncTotal);

      const scanBlock = content.createDiv({ cls: "obsidian-ntfy-lan-round-history-phase" });
      scanBlock.createEl("strong", { text: chinese ? "扫描" : "Scan" });
      if (localScanTotal > 0) scanBlock.createEl("progress", {
        cls: "obsidian-ntfy-lan-details-progress",
        attr: { max: String(localScanTotal), value: String(Math.min(localScanCompleted, localScanTotal)), "aria-label": chinese ? "本机检查进度" : "Local check progress" },
      });
      const localStats = scanBlock.createDiv({ cls: "obsidian-ntfy-lan-details-progress-stats" });
      localStats.createSpan({ text: chinese ? `本机已扫描 ${localScanCompleted}/${localScanTotal}` : `Local scanned ${localScanCompleted}/${localScanTotal}` });
      localStats.createSpan({ text: chinese ? `对端已扫描 ${remoteScanCompleted}/${remoteScanTotal}` : `Peer scanned ${remoteScanCompleted}/${remoteScanTotal}` });
      if (remoteScanTotal > 0) scanBlock.createEl("progress", {
        cls: "obsidian-ntfy-lan-details-progress",
        attr: { max: String(remoteScanTotal), value: String(Math.min(remoteScanCompleted, remoteScanTotal)), "aria-label": chinese ? "对端检查进度" : "Peer check progress" },
      });

      const syncBlock = content.createDiv({ cls: "obsidian-ntfy-lan-round-history-phase" });
      syncBlock.createEl("strong", { text: chinese ? "同步" : "Sync" });
      if (syncTotal > 0) syncBlock.createEl("progress", {
        cls: "obsidian-ntfy-lan-details-progress",
        attr: { max: String(syncTotal), value: String(Math.min(syncCompleted, syncTotal)), "aria-label": chinese ? "同步进度" : "Sync progress" },
      });
  ...
  renderScanSection(body, scan, remote, groups, chinese, progress, stage, stageDescriptions) {
    ...
    const label = `${chinese ? "本机已扫描" : "Local scanned"} ${scan.completed || 0} / ${chinese ? "本轮总扫描" : "round total"} ${scan.total || 0}`;
    ...
    scanStats.createSpan({ text: chinese ? `已扫描 ${scan.completed || 0} / 本轮总扫描 ${scan.total || 0}` : `Scanned ${scan.completed || 0} / Round total ${scan.total || 0}` });
    const scanFiles = Array.isArray(scan.files) ? scan.files : [];
    if (!groups.length) panel.createDiv({ cls: "obsidian-ntfy-lan-details-section-empty", text: chinese ? "已扫描 0/0" : "Scanned 0/0" });
  ...
  renderTransferSection(body, progress, files, groups, chinese, stage, stageDescriptions) {
    ...
    const label = hasTransferWork
      ...
      : stage === "complete"
        ...
        : stage === "packaging-manifest"
          ? (chinese ? "同步：正在封装清单" : "Sync: Packaging manifest")
          : stage === "planning"
  ...
  renderScanFile(parent, file, chinese) {
    ...
    const stateIcons = { pending: "circle", hashing: "circle-dot", cached: "database-zap", complete: "circle-check", skipped: "circle-minus", error: "circle-alert" };
  ...
  visibleFiles(files, limit) {
    ...
    add(files.map((file, index) => file.state === "syncing" || file.state === "hashing" || file.state === "deferred" || file.state === "error" ? index : -1).filter((index) => index >= 0));
  ...
  renderFile(parent, file, chinese) {
    ...
    const stateIcons = { pending: "circle", syncing: "circle-dot", complete: "circle-check", deferred: "refresh-cw", error: "circle-alert" };
    const row = parent.createDiv({ cls: `obsidian-ntfy-lan-details-file is-${file.state}` });
...
module.exports = class AndroidNtfyNotifierPlugin extends Plugin {
  async onload() {
    ...
    // its own single-file channel. active-leaf-change tracks which file is in
    // focus; editor-change fires on every content edit and debounces the sync.
    this.registerEvent(this.app.workspace.on("active-leaf-change", (leaf) => {
      const view = leaf && leaf.view;
      const file = view instanceof MarkdownView ? view.file : null;
      if (file) {
        this.lanSync?.notifyActiveEdit(file.path);
      } else {
        this.lanSync?.clearActiveEdit();
      }
    }));
    this.registerEvent(this.app.workspace.on("editor-change", (editor, info) => {
      const file = info && info.file;
      if (file) this.lanSync?.notifyActiveEdit(file.path);
    }));
  ...
  normalizeSettings(data) {
    ...
    settings.incomingAttachmentRetentionHours = Math.max(1, Math.min(8760, this.safePositiveNumber(
      ...
      settings.lanInboxRetentionHours || DEFAULT_SETTINGS.incomingAttachmentRetentionHours
    )));
    ...
    settings.lanInboxRetentionHours = settings.incomingAttachmentRetentionHours;
  ...
  async startLanSync() {
    ...
    const startGeneration = ++this.lanSyncStartGeneration;
    const adapter = this.app.vault.adapter;
    ...
    const service = new NtfyLanSync({
      ...
      storage: {
        ...
        listFilesChangedSince: async (since, includeConfigFolder) => {
          const changedFiles = this.app.vault.getFiles()
      ...
      onActivityChanged: () => {
        this.renderLanSyncStatusBar();
        this.lanSyncDetailsModal?.refresh();
        this.emitApiEvent("lan-activity", this.cloneApiValue(this.lanSyncActivitySnapshot({ includeScanFiles: false, includeTransferFiles: false })));
      },
      onPeersChanged: () => {
    ...
    try {
      await service.start();
      // Obsidian does not await onunload(). If a reload happened while the
      // service was binding sockets, discard this stale instance immediately
      // instead of leaving a second listener on port+1.
      if (startGeneration !== this.lanSyncStartGeneration || this.isUnloading || this.lanSync !== service) {
        await service.stop();
  ...
  renderLanSyncStatusBar() {
    ...
    if (!item || !text) return;
    const peers = this.lanSyncStatus().peers || [];
    // The status-bar takeover is shown only after this side has a currently
    // authenticated peer. If either endpoint disappears, listPeers() drains
    // on the next heartbeat/probe and both sides hide the icon together.
    const active = this.settings.lanSyncEnabled && this.lanSyncProgress.active && peers.length > 0;
    if (!active) {
      item.hidden = true;
      item.style.display = "none";
      this.remotelySaveStatusTakeover.restore();
    ...
    const peer = peers.find((candidate) => candidate.deviceId === this.lanSyncProgress.peerId) || peers[0];
  ...
  restartIncomingAttachmentCleanupTimer() {
    if (this.lanInboxCleanupTimer) window.clearInterval(this.lanInboxCleanupTimer);
    this.lanInboxCleanupTimer = null;
    ...
    this.lanInboxCleanupTimer = window.setInterval(() => void this.cleanupLanInboxAttachments(), intervalMs);
  ...
  incomingAttachmentMaxBytes(channelId = "") {
    const maxMb = String(channelId || "") === "lan"
      ? Math.max(1, Number(this.settings.lanSyncMaxFileMb || 512))
  ...
  async uniqueIncomingAttachmentPath(message, attachment) {
    ...
    await ensureNtfyLanFolder(this.app.vault.adapter, folder);
  ...
  async writeIncomingAttachment(message, attachment, bytes) {
    ...
    const hash = await hashLanSyncBytes(bytes);
    ...
    if (verified.byteLength !== bytes.byteLength || await hashLanSyncBytes(verified) !== hash) throw new Error("Downloaded attachment verification failed.");
  ...
  async downloadIncomingAttachment(message, attachment) {
    ...
    if (existingPath && await this.app.vault.adapter.exists(existingPath)) {
      ...
      if (isLanInboxAttachmentPath(existingPath) && existingPath !== localized.path) await this.app.vault.adapter.remove(existingPath).catch(() => undefined);
      return localized;
    }
  ...
  async ingestIncomingMessage(input = {}, options = {}) {
    ...
    const shouldLocalizeAttachments = message.attachments.some((attachment) => Boolean(
      ...
      || attachment.path && isLanInboxAttachmentPath(attachment.path) && this.incomingAttachmentRoot() !== ".trash/ntfy-inbox"
    ));
...
class AndroidNtfyNotifierSettingTab extends PluginSettingTab {
  ...
  renderLanSyncSettings(containerEl) {
    ...
    new Setting(group)
      ...
      .setDesc(this.uiText("可放入临时 Trash 收件目录，也可放入普通 Vault 文件夹。", "Store received files in the temporary Trash inbox or a regular Vault folder."))
      .addDropdown((dropdown) => dropdown
    ...
    new Setting(group)
      ...
      .addText((text) => text
        .setPlaceholder("附件/ntfy收件")
        .setValue(this.plugin.settings.incomingAttachmentFolder)
        .onChange(async (value) => {
    ...
    new Setting(group)
      ...
      .setDesc(this.uiText("默认 168 小时（7 天）；点“保存到 Vault”后的永久副本不在清理目录中。", "Defaults to 168 hours (7 days); copies saved to Vault are outside the cleanup folder."))
      .addText((text) => {
        text.inputEl.type = "number";
        text.inputEl.min = "1";
        text.inputEl.max = "8760";
        text.inputEl.step = "1";
        text.setValue(String(this.plugin.settings.incomingAttachmentRetentionHours)).onChange(async (value) => {
          this.plugin.settings.incomingAttachmentRetentionHours = Math.max(1, Math.min(8760, Number.parseInt(value, 10) || DEFAULT_SETTINGS.incomingAttachmentRetentionHours));
          this.plugin.settings.lanInboxRetentionHours = this.plugin.settings.incomingAttachmentRetentionHours;
          await this.plugin.saveSettings();
        });
      });
    new Setting(group)
      .setName(this.uiText("清理检查间隔（小时）", "Cleanup check interval (hours)"))
      .setDesc(this.uiText("决定多久检查一次过期收件文件，默认每 1 小时。", "Controls how often expired incoming files are checked; defaults to every hour."))
      .addText((text) => {
        text.inputEl.type = "number";
        text.inputEl.min = "1";
        text.inputEl.max = "720";
        text.inputEl.step = "1";
        text.setValue(String(this.plugin.settings.incomingAttachmentCleanupIntervalHours)).onChange(async (value) => {
          this.plugin.settings.incomingAttachmentCleanupIntervalHours = Math.max(1, Math.min(720, Number.parseInt(value, 10) || DEFAULT_SETTINGS.incomingAttachmentCleanupIntervalHours));
          await this.plugin.saveSettings();
          this.plugin.restartIncomingAttachmentCleanupTimer();
        });
      });
    new Setting(group)
      ...
      .setDesc(this.uiText("聊天附件点保存后复制到这里，不受自动清理影响。", "Saved chat attachments are copied here and are never affected by inbox cleanup."))
      .addText((text) => text
        .setPlaceholder("附件/ntfy")
        .setValue(this.plugin.settings.incomingAttachmentSavedFolder)
        .onChange(async (value) => {
