...
- The full Vault is synchronized by default, including safe configuration files. Workspace state, LAN identity, Remotely Save data, caches, `.git`, `node_modules`, and the temporary LAN inbox remain excluded. Desktop and mobile can both initiate manual and change-driven synchronization. File events use a path-only incremental pass immediately; the periodic check services pending paths, while an idle full-vault calibration runs no more than once every 10-60 minutes depending on the configured interval. Active scans and transfers are never interrupted.
