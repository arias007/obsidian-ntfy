...
- The full Vault is scanned and synchronized by default, including safe configuration files. Workspace state, LAN identity, Remotely Save data, caches, `.git`, `node_modules`, and the temporary LAN inbox remain excluded. Desktop and mobile can both initiate manual, change-driven, and periodic synchronization; periodic ownership rotates deterministically between connected peers to avoid duplicate scans. A periodic scan never interrupts an active scan or transfer.
